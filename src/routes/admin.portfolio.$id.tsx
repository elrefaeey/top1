    ...
    <div className="p-6 md:p-8 max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <AdminCard className="space-y-4">
          <AdminField label="العنوان" id="title"><input id="title" required value={form.title} onChange={(e) => patch({ title: e.target.value, slug: isNew ? slugify(e.target.value) : form.slug })} className={adminInputClass()} /></AdminField>
        ...
        <AdminSeoSection slug={form.slug} metaTitle={form.metaTitle} metaDescription={form.metaDescription} onSlug={(slug) => patch({ slug })} onMetaTitle={(v) => patch({ metaTitle: v })} onMetaDescription={(v) => patch({ metaDescription: v })} />
        <AdminFormActions saving={save.isPending} onDelete={isNew ? undefined : async () => { if (confirm("حذف؟")) { await remove.mutateAsync(id); navigate({ to: "/admin/portfolio" }); } }} />
      </form>
    </div>
  );
}
