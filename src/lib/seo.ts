...
/** Parse stored FAQPage JSON-LD from AI drafts; reject invalid payloads. */
export function parseStoredFaqSchema(raw?: string | null): ReturnType<typeof faqPageSchema> | null {
  const text = raw?.trim();
  if (!text) return null;
  try {
...
export function buildBlogPostHead(post: BlogPost, slugParam: string) {
  ...
  const path = `/blog/${slug}`;
  const title = post.metaTitle?.trim() || post.seoTitle?.trim() || `${post.title} | ${SITE_NAME}`;
  ...
  const faqLd = parseStoredFaqSchema(post.faqSchema);
